...
void detectInImage(Mat frame)
{
	...
	for (int i = 0; i < (int)frontalFaces.size(); i++) //for each face
	{
		...
		for (int j = 0; j < leftEyePos.size(); j++)
		{
			...
			leftEyeCenterFinal = snakuscule.runSnakuscule(grayFrame, leftEyeCenterInitial, &radius_left);
		...
		for (int j = 0; j < rightEyePos.size(); j++)
		{
			...
			rightEyeCenterFinal = snakuscule.runSnakuscule(grayFrame, rightEyeCenterInitial, &radius_right);
		...
		//Nose Detection
		vector<Rect> nosePos = noseDetector.storeNosePos(face);
		for (int k = 0; k < nosePos.size(); k++)
		{
		...
		if (rightEyeRightCorner != Point(0, 0) && leftEyeLeftCorner != Point(0, 0))
		{
			...
			horizontalShift = eyeGazeEstimator.horizontalShift(leftEyeCenterFinal, rightEyeCenterFinal, leftEyeLeftCorner, rightEyeRightCorner) / cornerDist;
			...
			verticalShift = eyeGazeEstimator.verticalShift(leftEyeCenterFinal, rightEyeCenterFinal, leftEyeLeftCorner, rightEyeRightCorner) / cornerDist;
